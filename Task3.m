...
worstAvgDelayService1 = 0;
worstAvgDelayService2 = 0;
...
while toc(t) < timeLimit
    ...
    [sol, maxLoad, Loads, Linkenergy] = HillClimbingLOAD(nNodes, Links, T, sP, nSP, sol, Loads, Linkenergy, L, Link_cap); 
    
    if  roundTripDelay < bestPropDelay
        bestSol= sol;
        bestLoad= maxLoad;
        bestLoadTime = toc(t);
        ...
        for f = 1:nFlows
            if sol(f) ~= 0
                ...
                if f <= 12
                    avgDelay = mean(roundTripDelayService1(roundTripDelayService1 ~= 0));
                    if avgDelay > worstAvgDelayService1
                        worstAvgDelayService1 = avgDelay;
                    end
                else
                    avgDelay = mean(roundTripDelayService2(roundTripDelayService2 ~= 0));
                    if avgDelay > worstAvgDelayService2
                        worstAvgDelayService2 = avgDelay;
                    end
                end
